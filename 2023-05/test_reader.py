...
class TestMetadata(reg.Metadata):
    ...
    def deconstruct_mosaic(self):
        ...
        self._slice_positions = (
            np.mgrid[: m_shape[0] : step_shape[0], : m_shape[1] : step_shape[1]]
            .reshape(2, -1)
            .T
        )
...
c1r.thumbnail = thumbnail.make_thumbnail(c1r, scale=0.5)
c2r.thumbnail = thumbnail.make_thumbnail(c2r, scale=0.5)
...
c2rr = TestReader(
    img=img2, tile_size=TILE_SIZE, angle=-np.rad2deg(cycle_tform.rotation)
)
c2rr.metadata._positions = np.fliplr(cycle_tform(np.fliplr(c2r.metadata.positions)))

# compute and add offsets since we use (0, 0) for rotation with resizing
h, w = c2rr.metadata._tile_size
rotation_offset = skimage.transform.AffineTransform(rotation=cycle_tform.rotation)(
    [(0, 0), (w, 0), (0, h), (w, h)]
).min(axis=0)[::-1]
...
v.add_points(np.fliplr(cycle_tform(np.fliplr(c2r.metadata.positions))))

for idx, pos in enumerate(c2rr.metadata.positions):
    v.add_image(
        c2rr.read(idx, 0), translate=pos, blending="additive", colormap="cividis"
    )

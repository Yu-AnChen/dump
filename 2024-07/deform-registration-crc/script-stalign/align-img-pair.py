...
scaling_factor = [1, 0.5, 1, 0.5, 0.5, 0.5, 0.5, 1, 1]
...
for nn, ff in zip(case_number, scaling_factor):
    print("processing", nn)

    ref = tifffile.imread(
        f"/Users/yuanchen/projects/STalign/docs/img_data/C{nn:02}-ref.tif"
    )
    moving = tifffile.imread(
        f"/Users/yuanchen/projects/STalign/docs/img_data/C{nn:02}-moving.tif"
    )

    matched_moving = skimage.exposure.match_histograms(moving, ref)

    if ff != 1:
        ref = skimage.transform.rescale(
            ref, ff, preserve_range=True, anti_aliasing=True
        )
        matched_moving = skimage.transform.rescale(
            matched_moving, ff, preserve_range=True, anti_aliasing=True
        )
